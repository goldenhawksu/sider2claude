...
export function saveSiderSession(
  ...
  if (!session) {
    ...
    consola.info('Created new Sider session:', {
    ...
    consola.info('Updated Sider session:', {
      cid: cid.substring(0, 12) + '...',
...
export function getNextParentMessageId(cid: string): string {
  ...
  if (!session) {
    consola.debug('No session found for CID:', cid.substring(0, 12) + '...');
    return '';
  }
  
  // 下次请求的 parent_message_id 应该是当前的 assistant_message_id
  consola.debug('Using parent message ID:', {
    cid: cid.substring(0, 12) + '...',
...
export function getOrCreateContinuousSession(): SiderSessionState {
  ...
  if (!session) {
    ...
    consola.info('Created new continuous conversation session');
  }
...
export function cleanupExpiredSiderSessions(maxAgeHours: number = 24): number {
  ...
  if (cleanedCount > 0) {
    consola.info('Cleaned up expired Sider sessions:', { count: cleanedCount });
  }
