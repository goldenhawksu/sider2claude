...
- **三档投递策略，由 `SIDER_STRATEGY` 选择**（也可在 `/stats` 页面随时切换）：
  ...
  放大成长时间不可用。但「不立即熔断」不等于「什么都不学」：实测存在第三类失败
  ——某模型在 Sider 侧根本接不了（`claude-fable-5` 恒返回 707，三次复现无一成功），
  它与时机、载荷都无关，频次与体量两个维度学不到任何东西，于是每个请求都白撞一次
  Sider 再 fallback。这类由 `recordSiderRejection` 的**持久性拒绝**通道兜住，判据
  保守到三条闸：只认明确的业务错误码（`siderCode ≠ 0`，网络抖动/超时不算）、要
  连续 3 次（任一次成功即清零）、停投可恢复（复用 1135 的 half-open 探测，退避
  从 5 分钟起）。改这里时不要把「一次成功即清零」去掉——没有它，跨越很长时间的
  孤立故障会攒够 3 次，把偶发放大成停投，正是这条通道要避免的事。
